...
%            'data','list','csi','spec','image', 'mrs', 'labels', twix
%            {filepath}, {filepathi}
% Input:
% CSIgui(data, label);
%
...
% Last Modified by GUIDE v2.5 28-Jan-2025 10:47:37
...
if exist([froot '\Files'],'dir') == 7
    addpath(froot); addpath([froot '\Files']); addpath([froot '\Images']);
    savepath;
else
...
gui.output = hObject;
% This field creates an ID for this CSIgui-instance.
...
%            'filepath', 'filepathi','labels', 'fp', 'fpi', 'twix'
...
% 2. MRSI > Color Scaling (2D) > Umbrella
gui.menubar.MRSI.ColorScale.Umbrella = ...
    uimenu(gui.menubar.MRSI.ColorScale.main, 'Label', 'Umbrella',...
    'Check', 'On','Separator','on',...
    'Callback', @CSI_2D_Scaling_Color_Umbrella_Set, 'Enable', 'On');
...
gui.menubar.MRSI.ColorScale.Colorbar = ...
    ...
    'Callback',@CSI_2D_Scaling_Color_showColorBar, 'Enable', 'On');
...
% 2. MRSI > Axis Scaling (2D) > Umbrella
gui.menubar.MRSI.AxisScale.Umbrella = ...
    uimenu(gui.menubar.MRSI.AxisScale.main, 'Label', 'Umbrella',...
    'Check', 'Off','Separator','on',...
    'Callback', @CSI_2D_Scaling_Axis_Umbrella_Set, 'Enable', 'On');
...
function delete_AppData(gui, auto)
% Delete CSI/MRI/CONV from appdata.
if nargin < 2, auto = 0; end

if ~auto
    elm = {'popup'}; 
    qry = {'Clear CSIgui cache and memory?'}; def = {{'Yes', 'No'}};
    uans = getInput(elm, qry, def, 'CSIgui - Delete Input');
    if isempty(uans), return; end
end

if auto || strcmp(uans{1}, 'Yes') 
    if isappdata(gui.CSIgui_main, 'csi')
        rmappdata(gui.CSIgui_main, 'csi');
    end
...
function button_ClearMemory_Callback(~, ~, gui)
% Before continuing, clear all memory from CSIgui
delete_AppData(gui, 1);
...
if strcmp(ext,'.dcm') || strcmp(ext,'.ima'), clear_log = 0; end

% Clear info in listbox.    
if clear_log
    set(gui.listbox_CSIinfo, 'String', {''}, 'value', 1);
end
...
elseif strcmp(ext,'.list') || strcmp(ext,'.data')              % LIST/DATA
    ...
    CSI_2D_Scaling_Xaxis(hObj, [], 1); % Automatic
...
%
% App-data: csi, mri & conv. Log-data resubmitted.

% Parse mat-file
[csi, mri, conv, log, gui, success] = csigui_parse_mat(fp, fn, gui);

% Save CSI data in app-data
setappdata(gui.CSIgui_main,'csi',csi);

% Store other appdata
if isstruct(mri), setappdata(gui.CSIgui_main, 'mri', mri); end
if isstruct(conv), setappdata(gui.CSIgui_main,'conv', conv); end

% Update log-data
% CSI_Log(log'); 
gui.listbox_CSIinfo.String = ...
    cat(1, gui.listbox_CSIinfo.String,{''}, log{:});
CSI_Log({'Log-data imported.'},{'See below.'});
...
if isfield(gui.inp,'data')
    ...
    CSI_2D_Scaling_Xaxis(gui.CSIgui_main,[],1); 
    gui = guidata(gui.CSIgui_main);
    csi = getappdata(gui.CSIgui_main,'csi'); 
...
while size(unique(order),2) ~= size(fn,2)    
    ...
    for nf = 1:size(fn,2)
        numbers{nf} = strsplit(num2str(...
            circshift(1:size(fn,2), -1 * (nf-1))));
    end
...
switch ext
    ...
    CSI_Log({'Reading all mat-files.'},...
            {'Please be patient, load-time depends on file-sizes.'});

        % -------- % Load all mat-files

        % Container variables and loop mat-file list
        mri = cell(1,numel(fn)); conv = cell(1,numel(fn));
        csi_dump = cell(1,numel(fn)); log = csi_dump;
        for kk = 1:size(fn,2)            
            % Load and parse mat-file
            [csi_dump{kk}, mri{kk}, conv{kk}, log{kk}, gui, success] = ...
                csigui_parse_mat(fp, fn{kk}, gui);

        end

        % -------- % Analyse mat-file data

        % Ask the user which file to use for default header info.
        % Save only data.raw from all other files
        qry = {'Which file-nfo and header should be stored?'};
        uans_foi = getUserInput_Popup(qry ,{fn});
        if isempty(uans_foi), CSI_Log({'Loading canceled.'},{''}); return; 
        end
        foi_ind = ismember(fn, uans_foi); % file_of_interest_index

        % Check inter-file size-compatibility
        data_sz = cellfun(...
        ...
        if sum(data_sz_check) ~= numel(fn)
            ...
            conv(fod_ind) = []; log(fod_ind) = [];
            
            % Show user which files are removed
            tmp = repmat({'Removed:'}, 1, numel(fod_ind));
            qry = ...
            [{'The file(s) below have incompatible data-dimensions.'},...
            tmp(:)'];
            CSI_Log({'% -------------------------------------- %'}, {''});
            CSI_Log(qry, {'', fn{fod_ind}});
            CSI_Log({'% -------------------------------------- %'}, {''});

            % Remove the wrong-sized files and update file-of-interest for
            % main header nfo.
            fn(fod_ind) = [];
        ...
        % -------- % Process Data 
        % Data-array dimensions are equal - a checksum is still in effect!
                
        % Concatenate data and add to struct of interest
        CSI_Log({'Combining mat-files and updating app-data.'},{''});

        % Main CSI-structure (chosen by user) used as starting-point
        csi = csi_dump{foi_ind}; nDim = numel(csi.data.labels);
        data = extractField(csi_dump, 'data.raw');  

        % Check for "file"-dimension in loaded data
        csi_dump_labels = extractField(csi_dump, 'data.labels');
        hasFile = cell2mat(cellfun(@(x) sum(contains(x, 'file')), ...
            csi_dump_labels, 'UniformOutput',false));
        label_name = 'file';
        if any(hasFile)           
            % Catenate dimension of interest
            hfoi = find(hasFile, 1, 'first'); % Has-file index of interest
            cat_dim = find(strcmp(csi_dump_labels{hfoi}, 'file'));

            % That needs to be done to the noise array too            
        else
            cat_dim = nDim + 1;
        end
  
        % Merge data by increasing dimensionality                                          
        csi.data.raw = cat(cat_dim, data{:});         
        csi.data.labels{cat_dim} = label_name;
        csi.data.dim = size(csi.data.raw);        
        csi.filepath = fp; csi.filename = strjoin(fn, ' | ');
        

        % -------- % Process Noise
        % NFO from csi-data will be used to merge noise-data.

        % Get noise data and check if indexes match
        noise_raw = extractField(csi_dump, 'data.noise',0);
        if ~isempty(noise_raw)            
            % Check dimensions are equal
            ndim = cellfun(@size, extractField(noise_raw, 'raw', 0),...
                'UniformOutput',false);             
            ndim_equal = isequal(ndim, repmat(ndim(1),1,numel(ndim)));
            if ndim_equal % Dimensions of noise match

                % Extract noise-data and labels
                noise_data = extractField(noise_raw,'raw', 0);
                labels = noise_raw{1}.labels;     
                
                % Write noise-data to struct and save in CSI struct.
                noise = struct; 
                noise.raw = cat(cat_dim, noise_data{:});
                noise.dim = size(noise.raw);
                noise.labels = labels; noise.labels(cat_dim) = {label_name};                

                % Store noise-data in csi-struct
                csi.data.noise = noise;             
            end
        end
        ...
        % Prepare and update log-data
        for kk = 1:numel(log)
            log_fn_inp = {'', '% -------------------------------- %', ...
                      fn{kk}, '% -------------------------------- %', ''};                
            gui.listbox_CSIinfo.String = ...
            cat(1, gui.listbox_CSIinfo.String, log_fn_inp', log{kk});
        end
...
% Calculate xaxis
CSI_2D_Scaling_Xaxis(gui.CSIgui_main);
...
if nargin == 1 || isempty(index) % If not or an empty index is given
    ...
    if isempty(uans), data_averaged = []; return; end
    index = csi_findDimLabel(labels, uans);
end
...
if  (size(win,1) > 1) % No apodization if window size equals 1.
    CSI_Log({['Filter ' uanstype{1} ' applied. Possible opts:']}, uopts);
end
...
if ~isappdata(gui.CSIgui_main, 'csi'), return; end
CSI_2D_Scaling_Xaxis(hObject, eventdata);
...
% ORDER FROM USER % -----------------------%
% Includes label maintenance

% Original number of labels.
ndim_ori = numel(csi.data.labels);

% Create additional labels: number of expended labels
lab = csi.data.labels; nexp = 2;
lab_qry = cat(2,lab, repmat({''}, 1, nexp)); strt = 65; 
for nl = 1:nexp
    n = 0;
    % create new label name and check if it does exist y/n   
    while sum(ismember(lab_qry, char([strt strt]))) == 1
        strt = strt + n; n = n+1;    
    end
    lab_qry{numel(lab) + nl} = char([strt strt]); n = strt;   
end

% Create query list for user input
ndim = ndim_ori + nexp;  numbers = cell(1,ndim);
new_order = ones(1,ndim);
while numel(unique(new_order)) ~= ndim

    for nf = 1:ndim
        numbers{nf} = strsplit(num2str(circshift(1:ndim, -1 * (nf-1))));
    end
    % Get new order from user.
    uans = getUserInput_Popup(lab_qry , numbers,...
           [gui.colors.main; gui.colors.text_main], 'Set load order.');
    if isempty(uans),CSI_Log({'Skipped data re-ordering.'},{''}); return; end
    
    % Str-ans to dbl-ans: new data order.
    new_order = str2double((uans));
end

% Corrrect for added dimensions if not used
expdims = ndim_ori+1:ndim_ori+nexp;
% Boolean if new added dimensions are on their original index
exp_bool = new_order(end-nexp+1:end) == expdims;
if  sum(exp_bool) == nexp
    new_order = new_order(1:end-nexp);
else
    % remove expended dimensions that arent used.
    cut_off = find((exp_bool == 0), 1, 'last');
    new_order = new_order(1:end+(cut_off-nexp));    
end

% Convert new-order for the permute operation
cur_order = 1:numel(new_order);
for kk = 1:numel(cur_order), cur_order(kk) = find(new_order == kk); end
new_order = cur_order;
...
% Get plot color
[clrs, clrs_data_range] = CSI_2D_Scaling_Color_Calculate(data_ylimits);  
...
% uans{1} - Noise decorrelation method
noiseMethod = uans{1};

% uans{2} - Source for noise-covariance matrix
nCovSource =  uans{2};
...
% decorrelation.
%
% If method, source and chan_ind arent given, userinput is required.

functag = 'Decorrelation:'; % Ease of CSI_Log

% Create backup
backup = gui.checkbox_backup.Value; 
if backup, CSI_backupSet(gui, 'Before noise decorrelation.'); end
...
csi.data.noise_cov = noise_cov;

switch method
    case 'Whitening'
        % Decorrelate signal (Whitening as done via WSVD)
        [csi.data.raw, csi.data.scaleMatrix] = ...
        csi_decorrelate_noise_whitening(csi.data.raw, chan_ind, noise_cov);

    case 'Cholesky'

        % Decorrelate signal (Cholensky)
        [csi.data.raw, csi.data.noise_cov_chol, csi.data.scaleMatrix] = ...
        csi_decorrelate_noise_chol(csi.data.raw, chan_ind, noise_cov);

    case 'ZCA'
        
        % Decorrelate signal (ZCA)
        [csi.data.raw, csi.data.scaleMatrix] = ...
        csi_decorrelate_noise_zca(csi.data.raw, chan_ind, noise_cov);
end
...
% Show nfo
CSI_Log({functag},{['Applied ' method]});
...
% Conver csi.data.raw to single or double depending on its current state.
% The button is named "Precision" - not single anymore.
...
doMRI = 0; if isappdata(gui.CSIgui_main, 'conv'), doMRI = 1; end
...
if isfield(csi.data,'voxelmask')
    ...
    permv(spat_dim) =  spat_dim(permvec); 
    csi.data.voxelmask = permute(csi.data.voxelmask, permv);
    csi.data.voxelmask = ...
        CSI_VoxelMask_Checksum(csi.data.voxelmask, size(csi.data.raw));
end
...
% k-space indexes
kspace = csi_findDimLabel(csi.data.labels,...
    {'kx','ky','kz','x', 'y', 'z', 'slice'});
kspace(isnan(kspace)) = [];

if isempty(kspace), kspace = 2:numel(csi.data.dim); end
...
% Interpolate ------------------------------------ Mask -- %
if isfield(csi.data, 'voxelmask')
    % Taking care of dim(1) = 1 and k-space dim change
    csi.data.voxelmask = csi_interpolate_voxelmask(...
        squeeze(single(csi.data.voxelmask)), new_dim, kspace-1, meth); 
    ndims = numel(size(csi.data.voxelmask));
    csi.data.voxelmask = permute(csi.data.voxelmask, [ndims+1 1:ndims]);
end
...
if isappdata(gui.CSIgui_main, 'conv')
    elm = {'popup'};
    qry = {'Recalculate coordinates and convert images?'};
    def = {{'Yes', 'No'}};
    uans = getInput(elm, qry, def);
    if ~isempty(uans) && strcmp(uans{1}, 'Yes')
        % Set coordinates CSI
        button_CSI_setCoordinates_Callback([], [], gui)
        % Convert MR images to CSI space (again)
        MRI_to_CSIspace(gui);    
    end
end
...
% Get requested shift
uans = getUserInput(...
    {'Numver of voxels to shift for each direction? (2D or 3D)'},...
    {'0.5 0.5 0.5'});
if isempty(uans), return; end
...
% Shift the data itself circularly to manipulate the data array.
...
% Get spatial (k)-space indexes
spat_lab = {'kx', 'ky', 'kz', 'x', 'y', 'z'};
spat_ind = csi_findDimLabel(csi.data.labels, spat_lab);
spat_ind(isnan(spat_ind)) = [];        
if isempty(spat_ind)
    CSI_Log({'Spatial dimensions not found.'},...
        {'Relabel indexes of interest to (k)x, (k)y or (k)z}'});
    return; 
end  
spat_lab = csi.data.labels(spat_ind);
spat_sz = size(csi.data.raw); spat_sz = max(spat_sz(spat_ind));

% Get shift direction plus amount.
elm = {'popup', 'popup'};
qry = {'Index to apply a circular data shift to:',...
       'Number of voxels to shift:'};
def = {spat_lab, {1:spat_sz}};
uans = getInput(elm, qry, def);
if isempty(uans), return; end

% Spatial index of interest
spoi = spat_ind(contains(spat_lab, uans{1}));
% Voxelshift
nshift = str2double(uans{2});

% Get direction (ud/lr/fw/bw)
elm = {'popup'}; qry = {'Shift direction:'};
if sum(strcmp(uans{1}, {'kx', 'x'})), def = {'Left', 'Right'};
elseif sum(strcmp(uans{1}, {'ky', 'y'})), def = {'Up', 'Down'};
elseif sum(strcmp(uans{1}, {'kz', 'z'})), def = {'Forward', 'Backward'};
end
uans = getInput(elm, qry, {def});
if isempty(uans), return; end

% Set direction to number of voxel shift.
if strcmpi(uans{1}, 'up') || strcmpi(uans{1}, 'left') || ...
        strcmpi(uans{1}, 'forward')
    nshift = -1.*nshift;
end

% SHIFT % ----------------------------------------------------- %
csi.data.raw = circshift(csi.data.raw, nshift, spoi);

% CLEAN UP % -------------------------------------------------- %
CSI_Log({'#Voxels shifted:'},{nshift});
CSI_Log({'Circular data-shift applied on index:'},csi.data.labels(spoi));
...
switch uans{1}
    case 'Single voxel'
        ...
        vox_ref = num2cell(str2double(strsplit(uans{1})));
...
qry = {'Data index/dimension to delete from:', ...
       'Index to delete (list entry (:) allowed):'};
def = { csi.data.labels(2:numel(size(csi.data.raw))), {'1'}};
uans = getInput(elm, qry, def, 'Delete MRS data');
...
if isempty(tmp{1})
    ...
    tbdeleted = [];
    for kk = 1:numel(tmp)
        isRange = strsplit(tmp{kk}, ':');
        if numel(isRange) > 1
            range = str2double(isRange{1}):str2double(isRange{2});
            tbdeleted(end+1:end+numel(range)) = range;
        else
            tbdeleted(end+1) = str2double(isRange{kk});
        end
    end
end
...
% Update noise if in data
if isfield(csi.data, 'noise') && isfield(csi.data.noise, 'raw')
    dimind{1} = 1:csi.data.noise.dim(1);
    csi.data.noise.raw = csi.data.noise.raw(dimind{:});
    csi.data.noise.dim = size(csi.data.noise.raw);
end
...
fid = csi_combine_reshape_revert(fid', permv, szr);
...
% uans{1} - Decorrelation method
do_NoiseDecorrelation = 0; ndmsg = 'None'; % None
switch uans{1}
...
% uans{4} - Average data
if strcmp(uans{4},'No'), do_avg = 0; else, do_avg = 1; end 

% uans{5} - Exclude channels
if strcmp(uans{5},'No'), do_excl = 0; else, do_excl = 1; end 
...
% Some CSI Log cosmetics and WSVD source.
CSI_Log({''},{''});
CSI_Log({'% ---------------------------------------------------- %'},...
        {''});
...
if do_PCA    
    % Run PCA
    button_CSI_PCA_Denoising_Callback(hobj, [], gui, 0);
    gui = guidata(hobj); csi = getappdata(gui.CSIgui_main, 'csi');
...
if do_NoiseData == 1                                      % Use noise-data
    % Get noise-cov using noise FID data
...
elseif do_NoiseData == 3                % Use stored ncov- / scale- matrix 
    ...
    elseif isfield(csi.data, 'noise_cov')
        ...
        CSI_Log({'No stored noise-cov matrix present.'}, ...
                {'Using voxel-data.'});        

        noise_cov = CSI_NoiseCov_usingData(...
            csi.data.raw(ind_all{:}), ind_cha);
...
for vi = 1:nvox
    ...
    % Noise covariance matrix / or scaleMatrix
    N = noise_cov{vi};
...
% Reorder to original order as before reshape (1/2).
csi.data.raw = csi_combine_reshape_revert(tmp_cell, permv, szr);
...
% DISPLAY INFO % ----------------------------- %
CSI_dataAs_Initiate(data_max, 'maximum/slice', gui, csi.data.labels);
...
for ri = 1:size(rem_index,1)
    ...
    sc.AlphaData =  repmat(0.3,size(x(:)));
...
%
% Uses csi_getSNR, csi_SNR() and csi_SNR_AUC;
%
...
% Show statistics nfo
stats = csi_statistics_of_volume(SNR);
...
% \\ Display Data
CSI_dataAs_Initiate(SNR, 'SNR', gui, csi.data.labels);

% --- Executed to get SNR for full volume
function [SNR, uans] = CSI_SNR_get(gui, auto)
% Calculate and return SNR for current MRS data set. 10.2025
%
% Options: 
%   Data unit selection, Use voxel or memory noise data, UAC or 
%   peak-maximum method, Noise mask size, Save to file
% 
% If auto == 1, default settings will be used without user input.
% uans contains als user-answers i.e. calculation settings used.
% uans{mask-size, noise-source, unit, method, strict AUC and save2file};
...
if nargin == 1, auto = 0; end

           % --------------- % USERINPUT % --------------- %
           % POI, mask-size, snr-method & display-method


% Get user input i.e. SNR calculation settings
[opts, uans] = CSI_SNR_opts(gui, auto);

              % --------------- % SNR % --------------- %

% Display Info %
CSI_Log({'Calculating SNR per voxel, please be patient.',...
         'Signal-unit & SNR-method:'},...
        {'', sprintf('%s | %s', uans{3}, uans{4})});

if opts.doAUC    
    % Calculate using noise mask and AUC
    SNR = csi_SNR_AUC(csi.data.raw, opts.range_none, ...
        opts.noise_inp, [], opts.snr_unit, opts.doStrict, 0);           
else
    % Calculate using noise mask and maximum peak value
    SNR = csi_SNR(csi.data.raw, ...
        opts.noise_inp, opts.snr_unit, opts.range_none);
end
...
% Save to file
if opts.doFile
    str_day = string(datetime('now','Format','yyyyMMddHHmm'));
    str_file = strjoin(['SNR_', str_day, '.mat'],'');
    save(str_file, 'SNR', 'opts.noise_inp', 'opts.doAUC');
end

% --- Executed to get SNR calculation options
function [opts, uans] = CSI_SNR_opts(gui, auto)
% Get all options from user for SNR calculations - returns processed user
% input structure and the user-answers cell array.
%
% Noise componennt is returned for the full volume if memory-noise is used
% instead of voxel-noise!
...
% POI: Peak of SNR
opts.range_none = CSI_getPeakOfInterest(csi.xaxis, 'Calculate SNR');
if isempty(opts.range_none), opts = []; uans = []; return; end

% SNR Noise mask
elm = {'edit', 'popup','popup', 'popup', 'popup', 'popup'};
qry = {'Size of noise mask:', 'Noise-source:', 'SNR Signal unit:', ...
       'Method:', 'Strict peak-range for AUC-method: ','Save to file:'};
def = {{round(csi.data.dim(1)/4)}, {'Noise-data', 'Voxel-data'},...
       {'Real', 'Magnitude', 'Imaginary'}, {'AUC', 'Maximum'},...
       {'Off', 'On'},{'No', 'Yes'}};
if auto, uans = cellfun(@(x) x(1), def);
else,    uans = getInput(elm, qry, def,'SNR');
end

% Return if user cancelled
if isempty(uans)
    opts = []; CSI_Log({'Skipped SNR calculations.'},{''}); return; 
end

% Mask-size user input
opts.mask_size = str2double(uans{1});

% SNR-method (real/magnitude) 
switch uans{3} % SNR method
    case 'Magnitude', opts.snr_unit = 0; 
    case 'Real',      opts.snr_unit = 1;     
    case 'Imaginary', opts.snr_unit = 2;
end

% SNR method (maximum/AUC)
opts.doAUC = 0; if strcmp(uans{4}, 'AUC'), opts.doAUC = 1; end

% Strict AUC method - see script for nfo. Default off. Not used for
% max-method.
opts.doStrict = 0; if strcmp(uans{5}, 'On'), opts.doStrict = 1; end

% SNR to file?
opts.doFile = 0; if strcmp(uans{6}, 'Yes'), opts.doFile = 1; end

% Noise-source and input
% Calculate it from voxel-data or use noise-data
opts.noise_inp = opts.mask_size;
switch uans{2}
    case 'Noise-data'
        % --- Get noise from file/memory

        % Checksum for noise-data
        CSI_Noise_Checksum(gui);
        csi = getappdata(gui.CSIgui_main,'csi');

        % Check if noise is available
        if isfield(csi.data, 'noise')
            if opts.snr_unit == 1
                opts.noise_inp = abs(std(real(csi.data.noise.raw), [], 1));
            elseif opts.snr_unit == 2
                opts.noise_inp = abs(std(imag(csi.data.noise.raw), [], 1));    
            else
                opts.noise_inp = abs(std(abs(csi.data.noise.raw),  [], 1));    
            end
        else        
...
% Shift every FID
data = cellfun(@csi_shift, data, 'uniform', 0);

% Convert to matrix.
csi.data.raw = cell2mat(data);
...
qry = {'Display type: ','Apply excluding voxel mask:', 'Mode:'};
def = {{'Map','Table','Histogram','Graph'},...
       {'Yes', 'No'},...
       {'Fast','Quality'}};
uans = getUserInput_Popup(qry, def, [] , data_tag);
if isempty(uans)
    CSI_Log({sprintf('%s mapping skipped.', data_tag)},{''}); return; 
end
...
% // Display mode
dispMode = lower(uans{3});
if strcmp(dataDisp, 'graph'), dispMode = 'Quality'; end
...
switch dispMode
    ...
    case 'fast'
        ...
        %  Check for converted image data
        if isappdata(gui.CSIgui_main, 'conv') 
            csi.conv = getappdata(gui.CSIgui_main,'conv');
        end
        
        opts.dataDisp = dataDisp; opts.doMask = doMask;        
        displayData(data, csi, data_tag, labels, opts);
        
    % PREVIOUS QUALITY METHOD (SLOW)    
    otherwise
        ...
        if doMask
            ...
            if isfield(csi.data, 'voxelmask')
                % Checksum of voxel-mask dimensions and data-size
                mask = CSI_VoxelMask_Checksum(csi.data.voxelmask, size(data));
        ...
        switch dataDisp
            ...
            case 'map'
                ...
                stats.mask = maskMsg; stats.source = 'displayed-data'; 
                stats.nfo = lower(data_tag); Statistics_Viewer(stats);

            case 'histogram'
                % Plot data as histogram - simple display.    
                fig = figure(); ax = axes(fig);
                mx = max(data(:), [], 'omitnan'); 
                md = median(data(:),'omitnan');  
                tmp = data(:);
...
% grid for plotting.
%
% data = a cell with an N-dimensional array. The 1st,2nd and 3rd dimension
...
if ~isnan(data_ylimits_color)
    ...
    [clrs, clrs_data_range] = CSI_2D_Scaling_Color_Calculate(data_ylimits_color);
else
    clrs = clr_li1; 
...
for sli = 1:size(data,4) % ----------------- %
    ...
    for ci = 1:plot_par.dim(1)     % Column loop.
        ...
        if     plot_par.scale_type_axis == 0 % to voxel, no change.             
        elseif plot_par.scale_type_axis == 1 || plot_par.scale_type_axis == 2 
                % Use pre-calculated limit relative to slice or volume
        ...
        if ylimit(2) <= ylimit(1), ylimit(2) = ylimit(1)+1; end
        if sum(isnan(ylimit)) > 0, ylimit = [0 1]; end
        % Set Ylimit
...
function [data, raw] = CSI_dataAsTable_getTableData(hobj)
...
% Get data
if ~strcmp(figobj.Tag, 'CSIgui_tableSNR') % Get data from maps
    ax = tgui.plot_par.ax(:);
    data = NaN(size(ax));
    for axi = 1:size(ax,1)
        if ~isempty(ax{axi})
    ...
    data = reshape(data, size(tgui.plot_par.ax));
else % get data from table
    [data, raw] = CSI_dataAsTable_getTableData(figobj);    
end
sz = size(data);
data = permute(data, [numel(sz)+1 1:numel(sz)]);
...
qry = {'Data range to show: ','Color scaling range: '};
def = {{'All', 'Current', 'Displayed'}, ...
...
elseif strcmpi(uans{1}, 'Displayed')
      % \\ Get current plotted slice index from CSIgui 2D-plot
    % \\ Get curent slice data from SNR_all array.
...
% SPATIAL INDEX % ----------- %
space_str = {'kx','ky','kz','x','y','z',  'slice'};
...
if isappdata(gui.CSIgui_main, 'conv')
    conv = getappdata(gui.CSIgui_main, 'conv');
    if ~isstruct(conv), conv = struct; end
end
...
% This function takes up to most time with increasing voxel-dimensions.
...
function plot_par = CSI_2D_getPlotSettings_AxisScaling(plot_par, gui, data_volume)
...
umbrella = CSI_2D_Scaling_Axis_Umbrella_Get(gui) && numel(plot_par.dim) > 3;
switch scaleby 
    case 'vox', plot_par.scale_type_axis = 0; 
    case 'sli', plot_par.scale_type_axis = 1; 

        % Unit
        tmp_data = CSI_getUnit(data_volume, plot_par.data_unit);
...
% --- Executed by CSI_2D_getPlotSettings
function plot_par = CSI_2D_getPlotSettings_ColorScaling(plot_par, gui, data_volume)
...
umbrella = CSI_2D_Scaling_Color_Umbrella_Get(gui) && numel(plot_par.dim) > 3;
switch scaleby
...
plot_par = CSI_2D_getPlotSettings_AxisScaling(plot_par, gui, data_volume);

% Scaling: Plot Color % ------------- %
% Create and calculate the color scale range settings.
plot_par = CSI_2D_getPlotSettings_ColorScaling(plot_par, gui, data_volume);
...
for ci = 1:plot_par.dim(1)                  % Col loop.
    for ri = 1:plot_par.dim(2)              % Row loop.
        ...
        ylimit = [min( plot_data_vox( color_scale_range ) ) ...
                  max( plot_data_vox( color_scale_range ) )];
        ...
        % Scale Y axis by limits of voxel, slice or volume.
        if plot_par.scale_type_axis == 0 % Voxel             
            ylimit = ...
                [min( plot_data_vox( full_scale_range ) )...
                 max( plot_data_vox( full_scale_range ) )];
        elseif plot_par.scale_type_axis ~= 0  % Slice (1) or Volume (2)
...
gridN = 5; gridW = 0.5; gridAlpha = 0.75; 
clr(4) = gridAlpha;
...
% --- Executed by user or CSI_plot2D_Initiate
...
% Add the frequency struct containing x-axis data for the 1D data.
...
if isfield(data2D.xaxis,'ppm')    
    ...
    data2D.axis.unit = 'none';   
end

% --- Add noise-component if available
% Requires CSIgui appdata "csi"
if ~exist('CSImain_obj', 'var')
    CSImain_obj = findobj('Tag','CSIgui_main', 'Name', 'CSIgui');
...
if isfield(csi.data, 'noise')
    try
        % Index of interest
        ind = num2cell(data2D.voxel.index);
        % Check if noise-array is matched to CSI-data
        CSI_Noise_Checksum(guidata(CSImain_obj));
        % Get updated appdata
        csi = getappdata(CSImain_obj, 'csi');
        
        data2D.noise = csi.data.noise.raw(:, ind{:});
    catch err
       fprintf(2, 'Noise checksum or data retrieval failed. \n');
       err_outp = getReport(err); disp(err_outp);
    end
end



% Slide Panel Follow Plot2D % ----------------------------------------- %
...
function CSI_2D_Scaling_Color_showColorBar(hObj,~)
...
% Set input
colorbarQ(plot_par.clrs,  plot_par.clrs_data_range);
...
function umbrella = CSI_2D_Scaling_Color_Umbrella_Get(gui)
menuChecked = get(gui.menubar.MRSI.ColorScale.Umbrella, 'checked');
umbrella = 0; if strcmp(menuChecked, 'on'), umbrella  = 1; end
...
function umbrella = CSI_2D_Scaling_Axis_Umbrella_Get(gui)
menuChecked = get(gui.menubar.MRSI.AxisScale.Umbrella, 'checked');
umbrella = 0; if strcmp(menuChecked, 'on'), umbrella  = 1; end
...
% --- Executed by user in menubar: CSI > Scaling Color
function CSI_2D_Scaling_Color_Umbrella_Set(hobj, ~)
% Get guidata
gui = guidata(hobj);

switch get(gui.menubar.MRSI.ColorScale.Umbrella, 'checked')
    case 'on',  setto = 'off';
    case 'off', setto = 'on';
end
set(gui.menubar.MRSI.ColorScale.Umbrella, 'checked', setto);
...
function CSI_2D_Scaling_Axis_Umbrella_Set(hobj, ~)
% Get guidata
gui = guidata(hobj);

switch get(gui.menubar.MRSI.AxisScale.Umbrella, 'checked')
    case 'on',  setto = 'off';
    case 'off', setto = 'on';
end
set(gui.menubar.MRSI.AxisScale.Umbrella, 'checked', setto);

% --- Executed by CSI_plot2D; y-axis color scaling.
function [clrs, clrs_range] = CSI_2D_Scaling_Color_Calculate(data_limits)
...
function CSI_2D_Scaling_Xaxis(hObject, ~, auto)
...
function [data, unit_str] = CSI_getUnit_data(data, gui)
% Returns the data according to the GUI popup menu plotUnit and the
% corresponding string.
% Uses CSI_getUnit.

unit_str = get(gui.popup_plotUnit,'String');
unit_str = unit_str{get(gui.popup_plotUnit,'Value')};
data = CSI_getUnit(data, unit_str);
...
if ppm
    ...
    CSI_Log({'Converted to data range-index: '},{poi});
end
...
% X axis
if isfield(appdata1D.axis ,'xlimit')
    appdata1D.axes.XLim = sort(appdata1D.axis.xlimit);
end
...
bInfo = {'Apply phasing corrections. Multiple methods available',...
         ...
         'Display noise component of the spectrum.'};
...
% Get peak of interest.
CSI_Log({'Executing automatic zeroth-order phase corrections.'},{''});
...
uans = getUserInput({'Requested sample size:'}, {size(data,1)*2,1});
if isempty(uans), return; end
...
% ----- % Userinput
% Multiple options available for SNR calculations
elm = {'popup', 'popup', 'popup', 'edit', 'popup', 'popup'};
qry = {'Calculate SNR:', 'SNR Method:', 'Noise-source:', ...
       'Noise mask size', 'Signal unit:', ...
       'Strict peak-range for AUC-method:'};
def = {{'Individual', 'Automatic'}, {'AUC', 'Maximum'}, ...
       {'in-memory','spectrum'}, {round(size(data,1)/4)},...
       {'Real','Magnitude', 'Imaginary'}, {'Off', 'On'}};
uans = getInput(elm, qry, def , 'SNR (1D)');
if isempty(uans), return; end

% Process input
% Single peak or automatic detection of multiple?
allPeaksOn = 0 ;switch(uans{1}), case 'Automatic', allPeaksOn = 1; end
% Use peak maximum (0) or area-under-curve (1) method?
snr_method = 0; switch(uans{2}), case 'AUC', snr_method = 1 ; end
% Noise-source: spectrum itself (0) or in memory (1) if available
noise_src = 0; switch(uans{3}), case 'in-memory', noise_src = 1; end
% Noise mask size
noise_mask_sz = str2double(uans{4}); 

% Signal unit for SNR calculations
snr_unit = 0; % Magnitude.
switch((uans{5}))
    case 'Real', snr_unit = 1; case 'Imaginary', snr_unit = 2; 
end

% Strict AUC calculations - see csi_AUC
doStrict = 0; if strcmp(uans{6}, 'On'), doStrict = 1; end

% ----- % Noise Compontent
% Get noise-component or use mask size itself.
noise_comp = noise_mask_sz;
if noise_src && isfield(appdata1D, 'noise')
    % Use noise in memory if available 
    if snr_unit == 1
        noise_comp = abs(std(real(appdata1D.noise), [], 1));
    elseif snr_unit == 2
        noise_comp = abs(std(imag(appdata1D.noise), [], 1));    
    else
        noise_comp = abs(std(abs(appdata1D.noise),  [], 1));    
    end
end
...
if allPeaksOn % ----------------------------------------------- All peaks
    ...
    uansAuto = getUserInput_Popup(qry,{def});
    if isempty(uansAuto), return; end
    ...
    switch uansAuto{1}
        ...
        case def{1} % --- Use Peaks.txt nfo ---------------------------- %
            ...
            def = peak_on;
            uansPeaks = ...
                getUserInput_Tick(qry, opt, 'Peaks to include:', def);
            if isempty(uansPeaks), return; end
        
            % Remove unselected peaks.
            peak_boundary(uansPeaks == 0, :) = [];
            peak_name(uansPeaks == 0) = [];
                        
            if isfield(appdata1D.xaxis,'ppm')
                ppm = appdata1D.axis.ppm; 
            else
                % Converts ppm-boundaries wrt center spectrum length.
                % This cannot be perfect - estimate of ppm2unitless.
        ...
        case def{2} % --- Use local maximum method --------------------- %
            ...
            uansPeaks = getUserInput_Popup({'Number of peaks:'},{{def}});
            if isempty(uansPeaks), return; end
            % #request peaks.
            nPeaks = str2double(uansPeaks{1});
            
            % Positions of peaks
            peak_pos_unit = csi_findPeaks(data, nPeaks)';

            % Update number of peaks.
            nPeaks = size(peak_pos_unit, 2);
            ...
            % Peak names (mockup)
            peak_name = cellfun(@int2str, num2cell(1:nPeaks), 'uniform', 0);
    end
    ...
    for kk = 1:np        
        ...
        if snr_method == 0 
            range = peak_pos_unit(kk) + [-2 2]; % Range of the peak

            % Calculate SNR using maximum
            snr(kk) = csi_SNR(data, noise_comp, snr_unit, range); 
            peak_est = {range};
        else
            range = peak_pos_unit(kk) + [-1 1]; % Range of the peak

            % Calculate using AUC        
            [snr(kk), peak_est] = csi_SNR_AUC(data, range,...
                noise_comp, noise_mask_sz, snr_unit, doStrict, 1);
            snr = snr./10;
        end 
        
        % Plot SNR as text at peak-max location
        tmp = peak_est{1}(1):peak_est{1}(2); 
               


        txt_plot_y = max(real(data(ceil(tmp),:)))+yst;
    ...
    if snr_method == 0 
        % Calculate SNR using maximum
        snr = csi_SNR(data, noise_comp, snr_unit, range); % Output SNR
        peak_est = {range};
    else 
        % Calculate using AUC        
        [snr, peak_est] = csi_SNR_AUC(data, range, ...
            noise_comp, noise_mask_sz, snr_unit, doStrict, 1);
    end
    ...
    text(appdata1D.axes, ind, double(val+yst), ...
...
% Display SNR.
CSI_Log({''},{''});
for pp = 1:numel(peak_name)
    inp = sprintf('%s - %3.3f',peak_name{pp}, snr(pp));
    CSI_Log({'SNR:'}, {inp});
end
CSI_Log({'Signal-unit & SNR-method:'},...
    {sprintf('%s | %s', uans{5}, uans{2})});
CSI_Log({'CSIgui-1D @ Voxel '}, {appdata1D.voxel.index});

% --- Executes when user presses "Noise" in panel_1D
function panel_1D_Noise(hobj, ~, ~)
...
% Y-axis limits, total value and text y-position
axlim = appdata1D.axes.YLim; 
axtot = diff(axlim); 
txty = (axtot .* 0.90) + axlim(1);

% Add 5% of the x-axis unit to the lower x-axis limit
txtx = appdata1D.axes.XLim(2) - diff(appdata1D.axes.XLim).*0.05;

% Noise in memory
if isfield(appdata1D,'noise')    
    noisevec{1} = appdata1D.noise; msksz(1) = size(noisevec{1},1)./2;
end

% Noise from voxel
msksz(2) = round((size(data,1)/4)/2); % Half mask!
noisevec{2} = data([1:msksz(2) size(data,1)-msksz(2)+1:size(data,1)]);    

% Remove memory-noise if not existant
if isempty(noisevec{1}), noisevec(1) = []; msksz(1) = [];  end

% Plot color string
clrstr = 'rg';

% Loop plot for noise-text and noise-vector
for kk = 1:numel(noisevec)

    % Noise STD
    noisestd = std(noisevec{kk}, 'omitnan');

    % Fix string for #decimals plot.
    [nZeros, acc] = numzeros(noisestd);
    if nZeros <= 0 || nZeros == acc
        txtval = strip(sprintf('%6.2f', val));    
    else
        prefix = '0'; if nZeros >= 9, prefix = ''; end
        txtval = sprintf('%3.3fe-%s%i', ...
                           noisestd*10.^(nZeros+1), prefix, nZeros+1);
    end
    
    % Prepare x-axis data for noisevector
    xdata = linspace(appdata1D.axes.XLim(1),appdata1D.axes.XLim(2),...
        size(data,1));
    xdata = xdata([1:msksz(1) size(data,1)-msksz(1)+1:size(data,1)]);
    
    % Hold current plot of axis.
    hold(appdata1D.axes, 'on');
    
    % Plot noise-vector on both sides of spectrum
    plot(appdata1D.axes, ...
        xdata(1:msksz(1)), real(noisevec{kk}(1:msksz(1)))',...
        '--', 'color', clrstr(kk));
    plot(appdata1D.axes,...
        xdata(msksz(1)+1:end), real(noisevec{kk}(msksz(1)+1:end))',...
        '--', 'color', clrstr(kk));
    
    % Update text-yaxis if more than 1 line
    if kk == 2, txty = (axtot .* 0.85) + axlim(1); end
    
    % Update noise-data origin string
    if kk == 1 && numel(noisevec) > 1, txtstr = '(memory)'; 
    else, txtstr = '(voxel)'; 
    end
    
    % Plot noise    
    text(appdata1D.axes, txtx, txty, ...
        sprintf('Noise %s: %s', txtstr, txtval), ...
        'FontSize', 8, 'FontWeight', 'Bold'); 
end
...
if strcmp(uans{1}, 'Intersection')
    auto = 0; if strcmp(uans{2}, 'Automatic'), auto = 1; end    
    panel_1D_FWHM_method_intersect(hObj, [], [], auto);
    return;
else
    uans = uans(2);
end
...
for pp = 1:size(peak_pos,1)
    ...
    [lw(pp), lwv(pp,:), lwp(pp,:)] = ...
     ...
     % [lw(pp), lwv(pp,:), lwp(pp,:)] = ...
     %     csi_linewidth_intersect(data, ax, peak_range);
...
function panel_1D_FWHM_method_intersect(hObj,~,~, auto)
...
if isfield(appdata1D.axis, axis_unit)
    switch axis_unit
        case 'time'    
            xlimit = appdata1D.axis.time([1 numel(appdata1D.axis.time)]);        
        case 'freq'
            xlimit = appdata1D.axis.freq([1 numel(appdata1D.axis.time)]);
        case 'none'
            xlimit = [1 numel(appdata1D.axis.none)];
        case 'ppm'
            xlimit = appdata1D.axis.ppm([1 numel(appdata1D.axis.time)]);
    end
end
...
switch uans{1}
    case 'Automatic'
        ...
        % Find peaks A
        [pks, locs, w] = findpeaks(datasmo,'NPeaks', 4); 
        ...
        dt = mean(diff(ax)); str = sprintf('Shifted: %1.6f | %3.0f', dt.*shiftval,...
            shiftval);
        text(ax(round(shiftval./2)), double(mm.*1.05), str,...
            'Color', [0.8 0.8 0.8]);
...
qry = {'Process noise data:', 'Delete a channel from noise-data:', ...
       ...
       'Combine data-channels:', 'Reorder indexes:'};
opt = repmat({{'Yes', 'No'}}, size(qry));
dans = ones(1,size(qry,2)); 
...
% Noise - Reorder indexes
if uans(8)
   button_CSI_ReorderDim_Callback(gui.CSIgui_main, [], gui); 
   gui = guidata(gui.CSIgui_main); 
end
...
function CSI_Noise_Checksum(gui)
% Return matched noise-data to CSI/MRSI data. Will require user input if
% data is not matched spatially or number of channel/coil-elements is
% unequal to csi-appdata. 
%
% Automatically permutes noise index-labels to match with csi index-labels.
% Returns NaN if no noise-data or csi-data is present and if errors occur.
%
% Fields of interest
% csi.data.noise.raw, csi.data.noise.labels, csi.data.raw, csi.data.labels

% Check available csi-appdata.
if ~isappdata(gui.CSIgui_main,'csi'), return; end
csi = getappdata(gui.CSIgui_main,'csi');

% Check available noise-data
if ~isfield(csi.data, 'noise'), return; end


% --- Match indexing --- %
% Assumed that index labels and #labels match with csi-data. Enforce
% correct order of dimensions using the labels.

% Unequal data-label vectors.
if ~isequal(csi.data.labels, csi.data.noise.labels)
    ...
    csi.data.noise.dim = size(csi.data.noise.raw);
    CSI_Log({'Matched noise index-order to csi-data:'}, ...
            {['New-order: ' strjoin(csi.data.noise.labels, ' | ')]});
           
    % Store to app-data
    setappdata(gui.CSIgui_main, 'csi', csi);
end

% --- Match spatial-index dimensions --- %
% Combining channel data and/or interpolation if necesary

% Check noise-dimensions
sz_data = size(csi.data.raw); sz_noise = size(csi.data.noise.raw);
if ~isequal(sz_data(2:end), sz_noise(2:end))  

    % BIG ERROR HERE - IF SQUEEZE(NOISE) but ~SQUEEZE(DATA)

    % Spatial and channel indexes
    lab_spat = {'kx','ky','kz', 'x', 'y', 'z'};
    ind_spat = csi_findDimLabel(csi.data.labels,lab_spat);
    ind_spat(isnan(ind_spat)) = [];
    ind_chan = csi_findDimLabel(csi.data.labels, {'chan'});  
    
    % Booleans
    doInterpolate = 1; 
    if isequal(sz_data(ind_spat), sz_noise(ind_spat))
        doInterpolate = 0; 
    end                
    doCombine = 0; 
    if ~isnan(ind_chan) && ind_chan <= numel(sz_noise) && ...
            sz_noise(ind_chan) ~= 1
        doCombine = 1; 
    end
    ...
    if doCombine
        % Ask to combine
        elm = {'popup'}; qry = {'Combine noise-data channels?'};
        def = {{'Yes', 'No'}}; uans = getInput(elm, qry, def, 'SNR-noise');
        if isempty(uans), return; end
    ...
    if doInterpolate
        % Ask to interpolate
        elm = {'popup'}; qry = {'Interpolate noise-data?'};
        def = {{'Yes', 'No'}}; uans = getInput(elm,qry,def,'SNR-noise');
        if isempty(uans), return; end
        doInterpolate = 1; if strcmp(uans{1}, 'No'), doInterpolate = 0; end
...
sz_data = size(csi.data.raw); sz_noise = size(csi.data.noise.raw);
if ~isequal(sz_data(2:end), sz_noise(2:end))
    % Process noise - by user
    CSI_Noise_Prepare(gui.CSIgui_main, gui);
end
...
elseif strcmp(csi.ext, 'dat') || isfield(csi,'twix')
    try
        ...
        TR = [0.3 0.5 0.65 0.8 1 1.2 1.5 2 4 6 8 10 12 14]; 
    end
...
for di = 1:double_iter
    ...
    if doDouble && di == 2 
        ...
        FAinit = 'Voxel dependent using first fit-iteration FA.';
    end
    ...
    yval = NaN(1, numel(TR));
    % barObj = loadBar(0, 'Fitting voxels...');
...
if isappdata(gui.CSIgui_main,'csi')
    ...
    if strcmpi(uans{3}, 'Yes')
        foi = {'noise_cov', 'noise_cov_chol', 'scaleMatrix'};
        for kk = 1:numel(foi)
            if isfield(csi.data, foi{kk})
                csi.data = rmfield(csi.data, foi{kk});
            end
        end
    end
    ...
    if strcmp(ext,'.fig')        
       ...
       if transparency
               export_fig([fp fn '_' number_title ext], ...
                '-transparent',['-' ftype],'-nocrop', '-m1', fig_obj);
       else
...
elseif strcmp(night,'on')
    ...
    colors.lines1     = [0.85 0.85 0];       % Static line color
    colors.lines2     = [1 0 0];             % Second line color
...
if ~isfield(csi, 'timer')
    ...
    % Safety for selection of peaks
    CSI_Log({'Executing timer-auto-click...'},{''});
    CSI_getPeakOfInterest(csi.xaxis, 'Set a peak-of-interest preset');
    csi = getappdata(gui.CSIgui_main, 'csi');
    
    t = timer('TimerFcn',@(~,~)auto_click, 'StartDelay', 1); 
    t.Period = 2; t.ExecutionMode = 'fixedRate';
    t.start;
    csi.timer = t;
    msg = 'enabled.';
else
...
switch uans{1}
    ...
    if isfield(csi.data, 'voxelmask')
        msz = size(csi.data.voxelmask);
        if numel(msz) > 3
            ind = num2cell(ones(1,numel(msz)-4)); 
            plot_par.mask = squeeze(csi.data.voxelmask(:,:,:,:,ind{:})); 
        else
            plot_par.mask = squeeze(csi.data.voxelmask); 
        end
    end
...
function mask = CSI_VoxelMask_Checksum(mask, dsz, sloi)
% Returns the voxelmask in the correct format for the given size. This can
% be used to ensure any masking isnt broken because of index/dimension
% changes due calculations and such.

if nargin < 3, sloi = {NaN}; end
...
% Check for non-spatial dimensions and if requested sizes match, plus
% selections of possible slice/plane of interest
if numel(msz) ~= numel(dsz) || ...
            sum(msz(2:end) ~= dsz(2:end)) < numel(dsz(2:end))

    % Repeat mask over non-spatial dimensions    
    dimrem = dsz(numel(msz)+1:numel(dsz));
    dimrem = [ones(1,numel(msz)) dimrem];
    mask = repmat(mask, dimrem); 

    % Select all mask-slices/planes of interest in the mask-array
    cell_ind = arrayfun(@(x) 1:x, dsz, 'UniformOutput', false);
    % First dimension of mask is only single value
    cell_ind{1} = 1;
    % Set specific slice/indexes of interest
    if sum(cellfun(@isnan, sloi)) == 0
        cell_ind(4:4+numel(sloi)-1) = sloi; 
    end
    % Cut mask
    mask = mask(cell_ind{:}); msz = size(mask);
end



% Last safety check
if sum(msz(2:end) ~= dsz(2:end))
    mask = zeros(dsz);
...
% Some log for user
CSI_Log({'Executing statistics calculations...'},{''});
...
qry = {'Calculate statistics over slice(s):', ...
       ...
       'Measure of interest:'};
def = {{'All', 'Current','Displayed', 'Select'},{'Yes','No'},{'No','Yes'},...
       {'Signal', 'SNR', 'Linewidth', 'Peak Location'}};
uans = getUserInput_Popup(qry, def);
if isempty(uans), return; end
...
% Select peak % ------------------------------------------------------- %
% LW requires full-spectrum and a peak will be selected during 
% LW-calculations, therefor, if user wants to select peak - itll be done
% later. Same goes for SNR.
if doPeak && (strcmp(uans{4}, 'Signal')   || strcmp(uans{4}, 'Peak Location')) 
...
% Here - take dimensions from label length - not from data itself because
% the indexes with size 1 are absent using size(x)....

        % Create queries for user (dim lists including "all" option)
        def = arrayfun(@(x) 1:x, dim(4:end), 'uniform', 0); 
        nqry = numel(def);
        for kk = 1:numel(def)
            def{kk} = ['All' arrayfun(@num2str, def{kk}, 'Uniform', 0)];       
        end   
        elm = repmat({'popup'}, 1, nqry); 
        qry = csi.data.labels(4:end);
        sel_uans = getInput(elm, qry, def, 'Statistics');
        if isempty(sel_uans), return; end

        % Message for LOG
        addStr = repmat(' %s |', 1, numel(sel_uans));
        msg = sprintf(['Selected data: ' addStr], sel_uans{:});

        % Process userinput
        sel_uans = str2double(sel_uans); non_nan = find(~isnan(sel_uans));
        doi_cell_layout = arrayfun(@(x) 1:x, dim, 'UniformOutput', false);
        doi_cell_layout(non_nan+3) = num2cell(sel_uans(non_nan));      
end
...
if doMask
    ...
    if isfield(csi.data, 'voxelmask')
        ...
        % Two thigns can happen here - either only sloi or sloi + higher
        if ~exist('sloi', 'var'), sloi = {NaN}; end
        mask = CSI_VoxelMask_Checksum(mask, size(doi), sloi);
        doi(mask) = (NaN + 1i*NaN);
...
switch uans{4}
    ...
    case 'SNR'
        % get user input for SNR calculations
        [opts, uans_SNR] = CSI_SNR_opts(gui);
        if isempty(uans_SNR), return; end

        % If noise_input is noise/voxel - take only for data of interest
        % using the previous indexing var doi_cell_layout;
        if numel(opts.noise_inp) > 1
            tmp = doi_cell_layout; tmp{1} = 1;
            opts.noise_inp = opts.noise_inp(tmp{:});
        end
        ...
        if opts.doAUC                
            % Calculate using noise mask and AUC
            doi = csi_SNR_AUC(doi, opts.range_none, ...
            opts.noise_inp, [], opts.snr_unit, opts.doStrict, 0);           
        else
            % Calculate using noise mask and maximum peak value
            doi = csi_SNR(doi, ...
        ...
        % Remove NANs from calculation and set mask to NaN again.
        doi(isnan(doi)) = 0; doi(mask) = NaN;

    case 'Linewidth'

        % Prep data
        ax = csi.xaxis.none;
        if isfield(csi.xaxis,'ppm'), ax = csi.xaxis.ppm; end 
        if ~exist('poi', 'var')
            poi = CSI_getPeakOfInterest(csi.xaxis,'Calculate statistics');
            [~, ~, poi] = ...
                CSI_getDataAtPeak(doi, ax, poi);
        end
        ...
        cell_layout = arrayfun(@ones, ones(1,size(sz(2:end),2)),sz(2:end),...
                        'UniformOutput',0);
        doi = mat2cell(doi, sz(1), cell_layout{:}); 

        % Calc LW
        doi = cellfun(@csi_linewidth_intersect, ...
            doi, repmat({ax},size(doi)), repmat({poi},size(doi)),...
            'UniformOutput', 0);
        doi = cellfun(@double, doi); doi(isnan(doi)) = 0; 
        if doMask, doi(mask) = NaN; end

    case 'Peak Location'

        % Ask user - in ppm or unitless
        if isfield(csi.xaxis, 'ppm')
            uans_pl = getUserInput_Popup({'Axis unit of interest:'},...
                                     {{'unitless', 'ppm'}});
            if isempty(uans_pl), return; end
            doPPM = 0;
            if strcmp(uans_pl{1}, 'ppm'), doPPM = 1; end
        end
        ...
        [~, doi] = max(real(doi));
        % Correct for peak-selection
        if doPeak, doi = doi + poi(1) - 1; end
        % Apply mask
        if doMask, doi(mask) = NaN; end

        % Apply conversion to ppm
        if doPPM
            ppm = csi.xaxis.ppm; nanBool = isnan(doi);           
            doi(nanBool) = 1; doi = ppm(doi); doi(nanBool) = NaN;            
        end
end


% Calculate statistics % ----------------------------------------------- %
stats = csi_statistics_of_volume(doi); stats.measure = uans{4};
Statistics_Viewer(stats);

% Log to user
CSI_Log({'Statistics calculated over: '}, {msg});

% --- Executes on button press in button_NFOviewer.
function button_NFOviewer_Callback(~, ~, gui)
% Open NFO viewer
...
% Load NFOviewer
CSI_Log({'Opening csi-struct in NFO-viewer.'}, {''}); 
NFOviewer(csi);
